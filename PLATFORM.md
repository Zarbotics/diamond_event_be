...
| UX2 | **Redesign booking creation and editing** | 🟡 | **Every section is out of the form; what is left is routing to the new workspace.** `EventStatFormModal.js` was **2,727 lines** holding every field a booking has, with `eventPayload.js` carrying another 556 of pricing rules that used to live inside it: one continuous form over the customer, the date, the venue, nine categories of food, décor with its options, extras, services, suppliers, the running order, payment and status. Everything visible at once regardless of relevance, dependencies between fields invisible, and no feedback that anything is wrong until submit. `booking/sections/` now holds all twelve — contact person, the couple, the running order, services, extras, the venue cascade, the customer, the event's identity, the food menu, the décor, the cost and payments, and the shared `FormSection` — as components that render inside the form's own `Form` context, so nothing was rewired, only moved. The form is **1,498 lines**. Each extraction is pinned by tests that assert the exact payload keys, because a renamed field does not fail loudly: the save succeeds and the value is quietly absent. **The lifts found six defects that were invisible while the code was one file:** neither décor price input was bound to the form, so a saved décor price was not on screen when the booking was reopened and re-typing it was the only reason it survived the next save; the décor notes were rendered inside a loop over the categories named "Themed Stage", so any other catalogue submitted no `txtDecoreRemarks` at all and wiped a saved one; two chips read "Click to apply as override price" and had no click handler; the guest, table and discount counts were `<Input type="number">` handing the form strings; a retired event type showed as a bare id; and a chain of four food-pricing functions plus a `foodMenuSelections` mirror of the form's own values was computed on every render and read by nothing (the same finding as D10d, in the part nobody had looked at). `booking/` also holds `BookingWorkspace` (rail, one section at a time, pinned summary), `SectionRail`, `BookingSummary` and `QuickCreate`, built and tested but **not yet routed to** — that is **UX2d**, and it changes how staff take and edit bookings day to day, so it needs agreement before it goes in. **Note the dependency:** the server now prices the booking (§5.10), so the redesign inherits a form that no longer has to compute money. |
