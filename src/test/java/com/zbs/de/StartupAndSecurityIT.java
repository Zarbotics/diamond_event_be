...
class StartupAndSecurityIT {
	...
	@Test
	@DisplayName("the database refuses two events with the same reference")
	void eventReferencesAreUnique() throws Exception {
		// The reference is generated by reading the highest code for the year
		// and adding one, so two customers starting a booking in the same
		// moment were both handed the same number — and nothing objected. It is
		// what a customer quotes on the phone and what staff search on.
		String url = System.getenv().getOrDefault("TEST_DB_URL",
				"jdbc:postgresql://localhost:5432/diamond_ev_test");
		String user = System.getenv().getOrDefault("TEST_DB_USERNAME", "postgres");
		String password = System.getenv().getOrDefault("TEST_DB_PASSWORD", "postgres");
		...
		try (Connection connection = DriverManager.getConnection(url, user, password)) {
			var index = connection.createStatement().executeQuery(
					"SELECT indexdef FROM pg_indexes WHERE schemaname = 'public' "
							+ "AND indexname = 'ux_event_master_code'");

			assertThat(index.next())
					.as("ux_event_master_code is missing, so duplicate references are still possible")
					.isTrue();
			assertThat(index.getString("indexdef"))
					.as("the index is not unique, so it prevents nothing")
					.contains("UNIQUE");

			// NULL is not a duplicate of NULL: rows predating code assignment
			// must not be blocked by this.
			assertThat(index.getString("indexdef"))
					.as("the index should be partial, so historic NULL codes still insert")
