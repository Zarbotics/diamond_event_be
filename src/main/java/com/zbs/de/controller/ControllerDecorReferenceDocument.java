...
@RestController
@RequestMapping("/decorRefrenceDocument")
@CrossOrigin(origins = "")
public class ControllerDecorReferenceDocument {

	private static final Logger LOGGER = LoggerFactory.getLogger(ControllerDecorItemMaster.class);

	@Autowired
	ServiceDecorReferenceDocument serviceDecorReferenceDocument;
	...
	public ResponseMessage getAllData(HttpServletRequest request) {
		...
		DtoResult dtoResult = serviceDecorReferenceDocument.getAll();
	...
	public ResponseMessage saveOrUpdate(@RequestBody DtoDecorReferenceDocument dto, HttpServletRequest request) {
		LOGGER.info("Saving or updating decor item: {}", dto);
		DtoResult dtoResult = serviceDecorReferenceDocument.saveOrUpdate(dto);
	...
	public ResponseMessage getById(@RequestBody DtoSearch dtoSearch) {
		LOGGER.info("Fetching DecoreReferenceDocument by ID: {}", dtoSearch.getId());
		DtoResult dtoResult = serviceDecorReferenceDocument.getById(dtoSearch.getId());
		if (dtoResult.getResult() != null) {
			return new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK, "Fetched successfully",
					dtoResult.getResult());
		}
